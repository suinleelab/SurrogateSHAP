...
class EmbedSequential(nn.Sequential, EmbedBlock):
    """Sequential container for embedding blocks."""
...
class ResBlock(EmbedBlock):
    """Residual block with timestep and class embeddings."""
...
#             torch.einsum(
#                 "bcq,bck->bqk",
#                 q.reshape(B * self.num_heads, ch, L),
#                 k.reshape(B * self.num_heads, ch, L),
#             )
#             / scale
#         )
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        if dtype is None:
            ...
            d = dtype.lower()
            if d in ("float32", "float", "fp32", "torch.float32"):
                self.dtype = torch.float32
            elif d in ("float16", "fp16", "torch.float16"):
                self.dtype = torch.float16
            elif d in ("bfloat16", "bf16", "torch.bfloat16"):
                self.dtype = torch.bfloat16
            else:
                raise TypeError(f"Unrecognized dtype string: {dtype}")
        else:
            # Try to coerce anything else (e.g., numpy dtypes)
            try:
                self.dtype = torch.dtype(dtype)
            except Exception:
                raise TypeError(
                    "dtype must be a torch.dtype, string, or None;"
                    f" got {type(dtype)}: {dtype}"
                )
