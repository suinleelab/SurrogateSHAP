"""
Fine-tuning script for Stable Diffusion for text2image with support for LoRA from [1]
[1] huggingface/diffusers/main/examples/text_to_image/train_text_to_image_lora.py
"""
...
import datasets
from configs.sd_config import LoraSparseUnlearningConfig, PromptConfig
from datasets import load_dataset
from src.datasets import (
    remove_data_by_loo,
    remove_index_by_datamodel,
    remove_index_by_shapley,
    remove_by_percentile
)
...
def save_model_card(
    ...
    """Saves a model card in the repo folder."""
    img_str = ""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["uniform", "shapley", "datamodel", "loo", "aoi", "percentile"],
        default=None,
    )
    parser.add_argument(
        "--datamodel_alpha",
    ...
    parser.add_argument(
        ...
        choices=["remove", "keep"],
        default="remove",
        help="Mode for percentile removal: 'remove' to remove indices, 'keep' to keep only those indices",
    )
    parser.add_argument(
        "--lora_dir",
        type=str,
...
def main():
    ...
    if args.removal_dist is not None:
        ...
        if args.removal_dist == "loo":
            ...
            # Extract method and model_behavior from percentile_file
            if args.percentile_file is not None:
                percentile_filename = os.path.basename(args.percentile_file).replace("_percentiles.json", "")
                # Expected format: artist_method_modelbehavior_percentiles.json
                # Remove 'artist_' prefix if present
                if percentile_filename.startswith("artist_"):
                    percentile_filename = percentile_filename[7:]  # Remove 'artist_' prefix
                removal_dir += f"/{percentile_filename}/{args.percentile_type}_{args.percentile_value}pct"
            else:
                removal_dir += f"/{args.percentile_type}_{args.percentile_value}pct"
        else:
    ...
    if args.method == "pruned_ft":
        ...
        if args.dataset == "artbench_post_impressionism":
            sparse_unlearning_config = (
                LoraSparseUnlearningConfig.artbench_post_impressionism_config
            )
        else:
    ...
    if args.lora_dir is None:
        ...
        if args.lora_steps is not None:
            weight_name = f"pytorch_lora_weights_{args.lora_steps}.safetensors"
        else:
    ...
    if args.removal_dist is not None or args.removal_rank_file is not None:
        ...
        if os.path.exists(removal_idx_file):
            ...
            remaining_idx = removal_idx_df["idx"][
                removal_idx_df["remaining"]
            ].to_numpy()
            ...
            if args.removal_dist == "shapley":
                ...
                print(f"Loaded {len(percentile_indices)} indices from {args.percentile_type} {args.percentile_value}% percentile")
                
                remaining_idx, removed_idx = remove_by_percentile(
                    removal_unit_df, percentile_indices, mode=args.percentile_mode
                )
            else:
                raise ValueError(
                    f"--removal_dist={args.removal_dist} has to be ['shapley', 'uniform', 'datamodel', 'loo', 'aoi', 'percentile']"
                )
    ...
    def unwrap_model(model):
        """Unwraps a model from potential wrappers"""
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
                        ...
                        try:
                            ...
                            unet_lora_state_dict = convert_state_dict_to_diffusers(
                                get_peft_model_state_dict(
                                    unwrapped_unet, adapter_name="default_0"
                                )
                            )
        ...
        if accelerator.is_main_process:
            ...
            if args.max_train_steps is None:
                ...
                time_record = (
                    f"{epoch},{epoch_time:.8f},{torch.cuda.get_device_name()}\n"
                )
                with open(time_file, "a") as f:
                    f.write(time_record)
    ...
    if accelerator.is_main_process:
        ...
        # remove checkpoint and save only lora weights
        ...
        print(
            f"Remove checkpoint files at {os.path.join(args.model_outdir, f'checkpoint*')}"
        )
        pattern = os.path.join(args.model_outdir, "checkpoint*")
